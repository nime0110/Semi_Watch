...
public class SmsSend_my extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
	    ...
	    // String api_key = "발급받은 본인의 API Key";  // 발급받은 본인 API Key
	      String api_key = "NCSO8DWGBM5KEEV3"; // 
	      
	    // String api_secret = "발급받은 본인의 API Secret";  // 발급받은 본인 API Secret 
	      String api_secret = "ERECOXLTND421WG0SCYTJBDEAZY6AVRN";  // 내거
	      
	      Message coolsms = new Message(api_key, api_secret);
	      // 라이브러리 파일이 필요함!! net.nurigo.java_sdk.api.Message 임. 
          ...
          System.out.println("~~~~ 확인용 mobile_code : " + mobile_code);
          // ~~~~ 확인용 certification_code : 
	      
	      
	      // == 4개 파라미터(to, from, type, text)는 필수사항이다. == 
	      HashMap<String, String> paraMap = new HashMap<>();
	      paraMap.put("to", newMoblie); // 수신번호
	      paraMap.put("from", "01095467026"); // 발신번호
	      // 2020년 10월 16일 이후로 발신번호 사전등록제로 인해 등록된 발신번호로만 문자를 보내실 수 있습니다
	      paraMap.put("type", "SMS"); // Message type ( SMS(단문), LMS(장문), MMS, ATA )
	      paraMap.put("text", mobile_code);
